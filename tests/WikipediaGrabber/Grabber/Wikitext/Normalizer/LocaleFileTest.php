...
class LocaleFileTest extends TestCase
{
    ...
    {
        $body = trim(file_get_contents(__DIR__ . '/LocaleFileTest/body.txt'));
        $section = new Section('Title', $body, 2);

        $normalize = trim(file_get_contents(__DIR__ . '/LocaleFileTest/normalize.txt'));
        $this->assertEquals($normalize, (new LocaleFile)->normalize($section));
    }
}
